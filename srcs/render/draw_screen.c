...
/*   Updated: 2019/05/04 15:14:38 by asamir-k         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_world_coords(t_bas *bas, t_plyr *player)
{
...
int		into_scree_coords(t_bas *bas, t_plyr *player, t_sector *sectors)
{
	if (bas->tz1 <= 0 && bas->tz2 <= 0)
		return(-1);
...
void	init_second_while(t_bas *bas, t_plyr *player)
{
bas->y1a = SCREEN_HEIGHT / 2 - (int)(yaw(bas->yceil, bas->tz1, player->yaw) * bas->yscale1);
			bas->y1b = SCREEN_HEIGHT / 2 - (int)(yaw(bas->yfloor, bas->tz1, player->yaw) * bas->yscale1);
			bas->y2a = SCREEN_HEIGHT / 2 - (int)(yaw(bas->yceil, bas->tz2, player->yaw) * bas->yscale2);
			bas->y2b = SCREEN_HEIGHT / 2 - (int)(yaw(bas->yfloor, bas->tz2, player->yaw) * bas->yscale2);
			bas->ny1a = SCREEN_HEIGHT / 2 - (int)(yaw(bas->nyceil, bas->tz1, player->yaw) * bas->yscale1);
			bas->ny1b = SCREEN_HEIGHT / 2 - (int)(yaw(bas->nyfloor, bas->tz1, player->yaw) * bas->yscale1);
			bas->ny2a = SCREEN_HEIGHT / 2 - (int)(yaw(bas->nyceil, bas->tz2, player->yaw) * bas->yscale2);
			bas->ny2b = SCREEN_HEIGHT / 2 - (int)(yaw(bas->nyfloor, bas->tz2, player->yaw) * bas->yscale2);
			bas->beginx = max(bas->x1, bas->now.sx1), bas->endx = min(bas->x2, bas->now.sx2);
			bas->x = bas->beginx;
}
void	nolose(t_bas *bas, t_data *data )
{
...
void	nolose2(t_bas *bas, t_data *data, t_plyr *player, t_sector *sectors)
{
...
void draw_screen(t_data *data, t_plyr *player, t_sector *sectors, unsigned int NumSectors)
{
	...
	while (bas.head != bas.tail)
	{
		...
		{
			get_world_coords(&bas, player);
			intersections1(&bas);
			if (into_scree_coords(&bas, player, sectors) == -1)
				continue;
			init_second_while(&bas, player);
			nolose2(&bas, data, player, sectors);
			++bas.renderedsectors[bas.now.sectorno];
		}
	}
}
