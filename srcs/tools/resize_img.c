...
/*   Updated: 2019/05/07 06:19:37 by ybouzgao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				draw_resized_column(t_data *data, t_object *sprite, t_wind wind, int ytop, int ybot)
{
	...
	while (j > 0 && res_j < sprite->y2)
	{
		...
		if (res_j == SCREEN_HEIGHT / 2 && sprite->x
				+ sprite->x_count == SCREEN_WIDTH / 2)
			data->aim = sprite->i;
		if (sprite->status == 1)
			put_pixel32(wind.screen, sprite->x + sprite->x_count, res_j,
					sprite->dead_img[(int)j * sprite->specs.width + (int)i]);
		if (sprite->status == 0)
			put_pixel32(wind.screen, sprite->x + sprite->x_count, res_j,
					sprite->img[(int)j * sprite->specs.width + (int)i]);
		j -= prop;
		res_j++;
	}
}
