...
/*   Updated: 2019/05/07 06:04:00 by dhorvill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "doom.h"

int			rbw(int x, unsigned int num_sectors)
{
	int			color;
	int			whichcolor;
	int			thiscolor;
	static int	rainbow[8] = {INDIGO, RED, ORANGE, YELLOW,
		GREEN, CYAN, BLUE, PURPLE};

	color = 0;
	whichcolor = (num_sectors) / 8;
	thiscolor = 0;
	while (color < 6)
	{
...
t_vector	Intersect(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4)
{
	...
	i.x = pv(pv(x1, y1, x2, y2), x1 - x2, pv(x3, y3, x4, y4), x3 - x4) /
		pv(x1 - x2, y1 - y2, x3 - x4, y3 - y4);
	i.y = pv(pv(x1, y1, x2, y2), y1 - y2, pv(x3, y3, x4, y4), y3 - y4) /
		pv(x1 - x2, y1 - y2, x3 - x4, y3 - y4);
	return (i);
}
...
{
	if (CROSSHAIR_X > data->sprite[0].x && CROSSHAIR_X < data->sprite[0].x
			+ data->sprite[0].size && CROSSHAIR_Y > data->sprite[0].y1 &&
